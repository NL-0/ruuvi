...
app.get('/all', cors(), (req, res) => {
influx.query('select mean(temperature), mean(rssi), mean(accelerationX), mean(accelerationY), mean(accelerationZ), mean(accelerationTotal) from ruuvi_measurements group by time(2s), mac fill(previous) order by desc limit 2').then(results => {
    //console.log(results)
    res.send(results)
  })
})
